...
b2Body * World::addPlayer(float x_pos, float y_pos, float x_size, float y_size, Filter_Data &data,Entity * foot_data) {
    ...
    polygonFixtureDef.friction = 0.1;
...
b2Body* World::addTriangle(float x_pos, float y_pos, float size, int angle ,bool static_obj,Filter_Data data){
    ...
    b2Vec2 position = b2Vec2(x_pos + DELTA_POSITION, y_pos + DELTA_POSITION);
    b2Vec2 delta;

    b2Vec2 vertices[3];
    if (angle == 45){
        delta = b2Vec2(0.5/3,-0.5/3);
    ...
    triangleBodyDef.position = position +  delta;
    ...
    triangleFixtureDef.friction = 1000000;
